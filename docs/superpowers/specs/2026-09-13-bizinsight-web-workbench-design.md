...
“发起分析”和会话记录合并为一个聊天卡片，不再把智能体回答放在输入卡片下方的独立区域。卡片从上到下由标题、可滚动消息记录、示例问题和固定输入区组成：
...
- 用户消息靠右，使用浅蓝气泡，用户头像位于气泡右侧；
- 智能体消息靠左，使用淡蓝或白色气泡，头像位于左侧并保留实际路由标签；
- 初始“准备就绪”提示显示在消息记录内，发送第一条消息后自然成为会话的一部分；
- 消息记录具有合理的最小与最大高度，长对话只在记录区域内部滚动；
- 新问题、思考状态和最终回答出现后，消息区域自动滚动到底部；
- 示例问题位于消息记录与输入框之间，点击后填入输入框；
- 输入框保持在聊天卡片底部，请求期间禁用重复发送；
- 分析执行流程和右侧报告预览的位置及行为不变。

### 精简顶栏与输入区

根据页面实际使用反馈，移除不参与操作的视觉元素，减少聊天区底部占用：

- 顶栏右侧不再显示 AgentScope、MCP、RAG、Tavily 状态标签、其后的分隔线和用户头像；
- 顶栏左侧品牌、Online 状态及产品说明保持不变；
- 删除示例问题整行和大尺寸多行文本框；
- 保留下方细条并将其改为真正的单行输入框；
- 细条内部从左到右为回形针图标、可输入文本区域和 `Ctrl + Enter 快速发送` 提示；
- “开始分析”按钮位于细条输入框外侧右边，与输入框同高并留出小间距；
- Enter 和 Ctrl+Enter 均发送当前问题，空问题仍不允许发送；
- 以上调整只修改页面结构、样式及键盘交互，不改变聊天 API、会话或智能体逻辑。

### 基于真实 telemetry 的执行流程

经营分析完成后，Web 适配层读取该会话和 run ID 对应的现有 telemetry，将其转换为稳定的结构化流程数据。浏览器不直接读取内部 telemetry 文件，核心智能体也不为页面增加额外埋点。

流程固定展示 Leader、财务销售、客户产品、项目交付、外部研究和 Evidence Reviewer 六个节点。每个节点的最终状态只能是：

- `success`：telemetry 存在该 Agent 且没有错误，显示绿色对钩；
...
普通问答、天气和单独 RAG 请求不伪装成经营分析流程：六个经营分析节点保持 `not_called`。请求等待期间仅显示中性“正在处理”，响应完成后一次性更新为 telemetry 证明的最终状态。
...
Worker 的指标算法、数据权限、SQL/RAG 工具、路由和 Reviewer 规则保持不变。首次 AgentScope 调用未生成合法 `Finding` 时，Worker 使用相同模型、系统提示词和受控工具创建干净的 Agent 上下文，并独立重试一次；正常成功请求不触发额外调用。重试成功时返回原有 `Finding`，重试仍失败时抛出包含 Worker 名称、总尝试次数和安全校验摘要的 `WorkerOutputError`，不得记录密钥、完整提示词或原始业务数据。自动化测试覆盖首次成功、AgentScope 内部纠正成功、独立重试恢复和两次调用均失败。
